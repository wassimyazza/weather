...
@section('title', 'Ajouter un Tableau - Admin')
@section('header', 'Ajouter un Tableau')

@section('content')
<div class="bg-white rounded-xl shadow-sm hover:shadow p-6 transition-all duration-300">
    <div class="mb-6">
        <h2 class="text-xl font-semibold text-gray-800">Nouveau Tableau</h2>
        <p class="text-sm text-gray-500 mt-1">Remplissez les informations ci-dessous pour ajouter un nouveau tableau à votre collection</p>
    </div>
    
    <form action="{{ route('admin.tableaux.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="4" 
                      ...
                      placeholder="Décrivez ce tableau..." required>{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
                ...
                <div class="relative">
                    <select id="user_id" name="user_id" 
                        ...
                        @foreach($painters as $painter)
                            <option value="{{ $painter->id }}" {{ old('user_id') == $painter->id ? 'selected' : '' }}>
                                {{ $painter->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="mb-6">
            <label for="images" class="block text-sm font-medium text-gray-700 mb-2">Images <span class="text-rose-500">*</span></label>
                ...
                reader.onload = function (event) {
                    ...
                    preview.innerHTML = `
                        <div class="relative group border rounded-lg overflow-hidden shadow-sm">
                            ...
                            ${i === 0 ? '<div class="absolute top-2 right-2 bg-amber-500 text-white text-xs px-2 py-1 rounded-full">Principal</div>' : ''}
                        </div>
                    `;
